    ...
    Decrypt(uint8_t const* src, int len)
{
    ...
    static bool inverseInitialized = false;
    if (!inverseInitialized) {
        for (int i = 0; i < 256; ++i) {
            inverseTable[sampEncrTable[i]] = static_cast<uint8_t>(i);
        }
        inverseInitialized = true;
    }
    ...
    for (int i = 0; i < len - 1; ++i) {
        const bool evenMask = (toggle == 0);
        toggle ^= 1;

        const uint8_t mask = evenMask
            ? static_cast<uint8_t>(((port ^ 0x5555u) >> 8) & 0xFFu)
            : static_cast<uint8_t>((port ^ 0x55u) & 0xFFu);

        const uint8_t mapped = static_cast<uint8_t>(src[i + 1] ^ mask);
        const uint8_t plain = inverseTable[mapped];
