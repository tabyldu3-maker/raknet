...
int SocketLayer::RecvFrom( const SOCKET s, RakPeer *rakPeer, int *errorCode )
{
	...
	if ( len != SOCKET_ERROR )
	{
		...
		if (decrypted) {
			if (SAMPRakNet::GetCore() && len > 1) {
				SAMPRakNet::GetCore()->printLn("[CLIENT] packet %02X len %d", decrypted[0], len - 1);
			}
...
int SocketLayer::SendTo( SOCKET s, const char *data, int length, unsigned int binaryAddress, unsigned short port )
{
	...
	do
	{
		if (SAMPRakNet::GetCore() && length > 0) {
			SAMPRakNet::GetCore()->printLn("[SEND] packet %02X len %d", (unsigned char)data[0], length);
		}
