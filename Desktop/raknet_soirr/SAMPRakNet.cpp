...
// S-Box таблица для нового формата
...
// Подготовка TEA ключей
static uint32_t PrepareKey(uint32_t key) {
    uint32_t x = key ^ 0x12913AFB;
    return (x >> 19) | (x << (32 - 19));
}
...
static void InitCrypto() {
    ...
    // Создание обратной S-Box таблицы
    for (int i = 0; i < 256; i++) {
        inv_sbox[sbox[i]] = i;
    }
    
    // Подготовка TEA ключей
    tea_key[0] = PrepareKey(0x8BB17BDA);
    tea_key[1] = PrepareKey(0x12048186);
    tea_key[2] = PrepareKey(0x121C4783);
    tea_key[3] = PrepareKey(0x74646B86);
    
    crypto_initialized = true;
}

// Построчное восстановление блоков (Unshuffle)
static void Unpermute(uint8_t* buf, int len, uint8_t* out) {
    int blockSize = (len - 5) / 4;
    uint8_t* indices = buf + 1;
    uint8_t* dataBlocks = buf + 5;
    
    for (int i = 0; i < 4; i++) {
        memcpy(out + i * blockSize, dataBlocks + indices[i] * blockSize, blockSize);
    }
}

// Алгоритм TEA дешифрации
...
uint8_t* SAMPRakNet::DecryptNewFormat(uint8_t const* src, int len) {
    ...
    // Копируем пакет во временный буфер для модификации
    if (len > MAXIMUM_MTU_SIZE) {
        return nullptr;
    }
    
    memcpy(decryptBuffer_, src, len);
    
    int dataLen = len;
    if (!DecryptPacket(decryptBuffer_, dataLen, GetPort())) {
        return nullptr;
    }
...
{
    // Проверка на новый формат пакета с маркером 0x1B
    if (len >= 5 && src[0] == 0x1B) {
        return DecryptNewFormat(src, len);
    }
    
    // Оригинальный алгоритм для обратной совместимости
...
// Тестовая функция для проверки нового алгоритма дешифрации
void SAMPRakNet::TestNewDecryption() {
    // Тестовый пакет с маркером 0x1B
    uint8_t test_packet[] = {
        0x1B, // Маркер
        0x57, 0x58, 0x59, 0x5A, // Индексы (после XOR должны стать 0,1,2,3)
        // Тестовые данные блоков...
    ...
    if (core_) {
        core_->printLn("Testing new decryption algorithm...");
        core_->printLn("Original packet length: %d bytes", packet_len);
        
        uint8_t* result = DecryptNewFormat(test_packet, packet_len);
        if (result) {
            core_->printLn("Decryption successful!");
            core_->printLn("First byte: 0x%02X", result[0]);
        } else {
            core_->printLn("Decryption failed!");
        }
    }
}
